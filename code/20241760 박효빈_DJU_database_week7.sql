...
SELECT 학번,
	...
	--ROUND(SUM(기말성적)::FLOAT / COUNT(*), 2)--MY SQL ROUND(숫자,자리수
FROM 수강2
GROUP BY 학번;
...
BEGIN 
    Count := 0; --변수 초기화
	OPEN ScoreListCursor;-- 커서를 열고 각 레코드(행,투플) 반복
	LOOP
...
INSERT INTO 남녀학생총수 SELECT '남', COUNT(*) FROM 학생2 WHERE 성별 = '여';
SELECT * FROM 남녀학생총수;
...
--3.트리거를 생성하기
CREATE OR REPLACE TRIGGER agter_insert_student
